...
internal class Overlay : IDisposable
{
	...
	private bool ShouldShowOnHover()
	{
		return _overlayConfig.ShowOnHover &&
		   (_overlayConfig.Hidden || HiddenByCombatFlags() || (_overlayConfig.HideInPvP && Services.ClientState.IsPvP)) &&
		...
		if (shouldShow || ShouldShowOnHover())
		{
			...
			else
			{
				if (_texErrorIcon is not null)
				{
					...
					float size = MathF.Min(_size.X - lineHeight * 3, _size.Y - lineHeight * 3);
					ImGui.NewLine();
